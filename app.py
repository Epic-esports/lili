        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 200 and data.get('msg') == 'id_found':
                ...
                return {
                    ...
                    'is_banned': player_data.get('is_banned', '0'),
                    'ban_period': player_data.get('period', 'Unknown'),
                    'region': player_data.get('region', 'Unknown'),
                    'last_login': player_data.get('last_login', 'Unknown'),
                    'created_at': player_data.get('createAt', 'Unknown'),
                    'ban_status': 'Banned' if player_data.get('is_banned') == '1' else 'Not Banned'
                }
            else:
                return {"error": f"Player not found or API error: {data.get('msg', 'Unknown error')}"}
        else:
...
def newinfo(uid):
    try:
        url = f"https://like2.vercel.app/player-info?uid={uid}&server=BD&key=rifat1122"
        response = requests.get(url)

        if response.status_code == 200:
            api_data = response.json()
            print(f"Response Data: {api_data}")  # Print data for verification

            # Check if data exists in the new response format
            if "data" in api_data and isinstance(api_data["data"], dict):
                data = api_data["data"]
                
                # Check for basicInfo in the nested data structure
                if "basicInfo" in data and isinstance(data["basicInfo"], dict):
                    # Keep the original structure but use the new API data
                    pass
                else:
                    print("Error: 'basicInfo' key not found or empty")
                    return {"status": "wrong_id"}

                # Check for clanBasicInfo
                if "clanBasicInfo" not in data or not isinstance(data["clanBasicInfo"], dict):
                    data["clanBasicInfo"] = "false"

                # Check for captainBasicInfo
                if "captainBasicInfo" not in data or not isinstance(data["captainBasicInfo"], dict):
                    data["captainBasicInfo"] = "false"

                # Add additional info from the new API structure
                data["credits"] = api_data.get("credits", "Unknown")
                data["timestamp"] = api_data.get("timestamp", "Unknown")

                return {"status": "ok", "info": data}
            else:
                print("Error: 'data' key not found or empty in API response")
                return {"status": "wrong_id"}
...
def send_spam(uid):
    try:
        ...
        api_url = f"https://spam-dusky.vercel.app/send_requests?uid={uid}&server=ind"
        ...
        if response.status_code == 200:
            ...
            player_name = api_data.get('player', 'Unknown')
            success_count = api_data.get('success_count', 0)
            failed_count = api_data.get('failed_count', 0)
            total_tokens = api_data.get('total_tokens_used', 0)
            timestamp = api_data.get('timestamp', 'Unknown')
            server = api_data.get('server', 'Unknown')
            
            if success_count > 0:
                # Success case
                return (
                    f"{generate_random_color()}┈━═☆Friend Requests☆═━┈\n"
                    f"[B][FFA500] Player: [FFFFFF] {player_name}\n"
                    f"[B][FFA500] Player ID: [FFFFFF] {fix_num(uid)}\n"
                    f"[B][00FF00] Success: [FFFFFF] {success_count} ✅\n"
                    f"[B][FF0000] Failed: [FFFFFF] {failed_count}\n"
                    f"[B][FFA500] Total Tokens: [FFFFFF] {total_tokens}\n"
                    f"[B][FFA500] Server: [FFFFFF] {server}\n"
                    f"[B][FFA500] Time: [FFFFFF] {timestamp}\n"
                     f"[B][FFA500] Credits: [FFFFFF]Adil★\n"
                    f"┈━═☆☯☯☆═━┈"
                )
            else:
                # All failed case
                return (
                    f"[FF0000]┈━═☆Friend Requests☆═━┈\n"
                    f"[B][FFA500] Player: [FFFFFF] {player_name}\n"
                    f"[B][FFA500] Player ID: [FFFFFF] {fix_num(uid)}\n"
                    f"[B][FF0000] All requests failed ❌\n"
                    f"[B][FF0000] Failed: [FFFFFF] {failed_count}\n"
                    f"[B][FFA500] Total Tokens: [FFFFFF] {total_tokens}\n"
                    f"[B][FFA500] Server: [FFFFFF] {server}\n"
                     f"[B][FFA500] Credits: [FFFFFF]Adil★\n"
            ...
            return (
                f"[FF0000]-----------------------------------\n"
                f"[B][FFA500] API request failed ({response.status_code})\n"
                f"-----------------------------------\n"
            )
        ...
        return (
            f"[FF0000]-----------------------------------\n"
            f"[B][FFA500] Failed to connect to the server:\n"
            f"{str(e)}\n"
            f"-----------------------------------\n"
        )
...
def send_likes(uid):
    try:
        response = requests.get(
            f"https://like-neon-rho.vercel.app/like?uid={uid}&server=bd",
            timeout=100
        )
        ...
        if response.status_code == 200:
            ...
            if likes_added == 0 or likes_before == likes_after:
                # No likes added (daily limit reached)
                return {
                    "status": "failed",
                    "message": (
                        f"[C][B][FF0000]________\n"
                        f"[B][FFA500]❌ Daily limit for sending likes reached!\n"
                        f"[B][FFA500] Try again after 24 hours.\n"
                        f"[B][FFA500] Player: [FFFFFF]{player_name}\n"
                        f"[B][FFA500] LikesBefore: [FFFFFF]{likes_before}\n"
                        f"[B][FFA500] LikesAfter: [FFFFFF]{likes_after}\n"
                         f"[B][FFA500] Credits: [FFFFFF]Adil★\n"
                        f"________"
                    )
                }
                ...
                return {
                    "status": "ok",
                    "message": (
                        f"[C][B][00FF00]┈━═☆Player Like☆═━┈\n"
                        f"[B][FFA500]✅ Likes Sent Successfully!\n"
                        f"[B][FFA500] Player Name: [FFFFFF]{player_name}\n"
                        f"[B][FFA500] Likes Added: [FFFFFF]{likes_added}\n"
                        f"[B][FFA500] Likes Before: [FFFFFF]{likes_before}\n"
                        f"[B][FFA500] Likes After: [FFFFFF]{likes_after}\n"
                        f"[B][FFA500] Credits: [FFFFFF]Adil★\n"
                        f"┈━═☆☯☯☆═━┈"
                    )
                }
            ...
            return {
                ...
                "message": (
                    f"[C][B][FF0000]________\n"
                    f"[B][FFA500]❌ Error Sending Likes!\n"
                    f"[B][FFA500] Server Response Code: [FFFFFF]{response.status_code}\n"
                    f"[B][FFA500] Please check API connection.\n"
                    f"________"
                )
            }
        ...
        return {
            ...
            "message": (
                f"[C][B][FF0000]________\n"
                f"[B][FFA500]❌ Connection Failed!\n"
                f"[B][FFA500] API server might be offline.\n"
                f"________"
            )
        }
...
class FF_CLIENT(threading.Thread):
    ...
    def createpacketinfo(self, idddd):
        ...
        return bytes.fromhex(final_packet)
    def accept_sq(self, hashteam, idplayer, owner):
        fields = {
        1: 4,
        2: {
            1: int(owner),
            ...
            if "1200" in data.hex()[0:4]:
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                            f"""[FFFFFF][b][c]✨Welcome Brother! I am Read TO Help You 😊  ✨[/b]

[FFFFFF][c]Show All Command List, [c]yeh command bhejo:  
...
Are you thinking about getting the bot at a good price?

Are you thinking about getting a panel with no restrictions?

Are you thinking about getting a server in your name with a dashboard?

All of this is available — contact me!

[b][i][A5E2CFٍ]Discord: @sheikhadil0702[/b]

[b][c][1E90FF]YouTube : @A15 CORPORATION[FFFFFF]

Enjoy the bot my friend.......

[C][B][FF6347] Developer   ADIL""", uid
                    )
                )
            ...
            if "1200" in data.hex()[0:4] and b"/3" in data:
                ...
                if uid:
                    clients.send(
                        self.GenResponsMsg(
                            f"[C][B][1E90FF]┈━═☆Player Group☆═━┈\n\n\n\nRequest Successfully sent now You Add Into 3 Player Group\n\n\n\n┈━═☆☯☯☆═━┈",
                            uid
                        )
                    )
            ...
            if "1200" in data.hex()[0:4] and b"/5" in data:
                ...
                if uid:
                    clients.send(
                        self.GenResponsMsg(
                            f"[C][B][1E90FF]┈━═☆Player Group☆═━┈\n\n\n\nRequest Successfully sent now You Add Into 5 Player Group\n\n\n\n┈━═☆☯☯☆═━┈",
                            uid))
            ...
            if "1200" in data.hex()[0:4] and b"/6" in data:
                ...
                if uid:
                    clients.send(
                        self.GenResponsMsg(
                  f"[C][B][1E90FF]┈━═☆Player Group☆═━┈\n\n\n\nRequest Successfully sent now You Add Into 6 Player Group\n\n\n\n┈━═☆☯☯☆═━┈",
                            uid))
            ...
            if "1200" in data.hex()[0:4] and b"/status" in data:
                try:
                    ...
                    if room_data:
                        ...
                        clients.send(self.GenResponsMsg("[C][B][FF0000] Plaease Enter Your Uid", uid))  
                except Exception as e:
            ...
            if "1200" in data.hex()[0:4] and b"WELCOME TO [FFFFF00] ADIL [ffffff]BOT" in data:
            	pass
            else:
	            ...
	            if "1200" in data.hex()[0:4] and b"/spam" in data:
	                ...
	                if len(command_split) > 1:
	                    ...
	                    clients.send(
	                    self.GenResponsMsg(
	                        f"{generate_random_color()}Sending Spam Request", uid
	                    )
	                )
	                    ...
	                    if len(command_split) <= 1 or not command_split[1].strip():  # ✅ إذا لم يتم إدخال ID
	                        ...
	                        if not uid:
	                            ...
	                            if 'info' not in info_response or info_response['status'] != "ok":
	                                ...
	                                basic_info = infoo['basicInfo']
	                                clan_info = infoo.get('clanBasicInfo', "false")
	                                clan_admin = infoo.get('captainBasicInfo', "false")

	                                if clan_info == "false":
	                                    clan_info_text = "\nPlayer Not In Clan\n"
	                                else:
	                                    clan_info_text = (
	                                        f"[B][FFA500]• Clan Info :\n"
	                                        f"[B][FFA500]• Clan ID : [FFFFFF] {fix_num(clan_info.get('clanId', 'N/A'))}\n"
	                                        f"[B][FFA500]• Name: [FFFFFF]{clan_info.get('clanName', 'N/A')}\n"
	                                        f"[B][FFA500]• Members: [FFFFFF]{clan_info.get('memberNum', 0)}\n"
	                                        f"[B][FFA500]• Level: [FFFFFF]{clan_info.get('clanLevel', 0)}\n"
                                            f"[C][B][CF09EA]FALCON x64\n"
	                                       f"[C][B][00FF00]┈━═☆☯☯☆═━┈\n"
	                                ...
	                                bio = infoo.get('socialInfo', {}).get('signature', "No bio available").replace("|", " ")
	                                br_rank = fix_num(basic_info['rankingPoints'])
    ...
    f"[B][FFA500]• Bio: [FFFFFF]{bio}\n"
	                            ...
	                            if 'info' not in info_response or info_response['status'] != "ok":
	                                ...
	                                if clan_info == "false":
	                                    ...
	                                    clan_info_text = (
	                                        ...
	                                        "Clan Admin Info : \n"
	                                        f"ID : {fix_num(clan_admin.get('accountId', 'N/A'))}\n"
	                                        f"Name : {clan_admin.get('nickname', 'N/A')}\n"
	                                        f"Exp : {clan_admin.get('exp', 'N/A')}\n"
	                                        f"Level : {clan_admin.get('level', 'N/A')}\n"
	                                        f"Ranked (Br) Score : {fix_num(clan_admin.get('rankingPoints', 0))}\n"
	                                    )
	            ...
	            if "1200" in data.hex()[0:4] and b"/likes" in data:
	                    ...
	                    clients.send(
	                    self.GenResponsMsg(
	                        f"{generate_random_color()}Processing the request!", uid
	                    )
	                )
	            ...
	            if "1200" in data.hex()[0:4] and b"/check" in data:
	                   try:
	                   	...
	                   	clients.send(
	                   	self.GenResponsMsg(
                            f"{generate_random_color()}Processing Your Request", uid
                        )
                    )
	                   	if len(command_split) > 1:
	                   	   ...
	                   	   if 'error' in banned_status:
	                   	       response_message = f"[B][FF0000] Error: [FFFFFF] {banned_status['error']}"
	                   	   else:
	                   	       status = banned_status.get('ban_status', 'Unknown')
	                   	       player_name = banned_status.get('nickname', 'Unknown')
	                   	       ban_period = banned_status.get('ban_period', 'Unknown')
	                   	       region = banned_status.get('region', 'Unknown')
	                   	       last_login = banned_status.get('last_login', 'Unknown')
                               
	                   	       response_message = (
                                f"[B][FFA500] Player Name:[FFFFFF] {player_name}\n"
                                f"[B][FFA500] Player ID : [FFFFFF] {player_id}\n"
                                f"[B][FFA500] Status: [FFFFFF] {status}\n"
                                f"[B][FFA500] Region: [FFFFFF] {region}\n"
                                f"[B][FFA500] Last Login: [FFFFFF] {last_login}\n"
                            )
            ...
            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
	                    self.GenResponsMsg(
		                        f"""[B][C][FFFF00] WELCOME TO [ffffff]ADIL BOT GAME BOT  !! 

[C][B][00FF00]/🤔likes [id] -> [C][B][FFFFFF]You Want To Like 100 For Your Account

[FF0000] /🤔info [id] -> [C][B][FFFFFF]Any Player Get Info

[00FF00َ] /🤔status[id] ->  [C][B][FFFFFF]Player Status Show

		-------------------------------

[FF0000] /🤔spam [id] -> [C][B][FFFFFF] 

[00FF00] /🤔check [id] -> [C][B][FFFFFF]

[40E0D0] /🤔region [id] ->        [C][B][FFFFFF]
...
[00FF00] /🤔x [id] -> [C][B][FFFFFF] Spam Request

[40E0D0] /🤔room [id] -> [C][B][FFFFFF] Spam Request 

[00FF00َ] /🤔admin -> [C][B][FFFFFF]Bot Admin Information

 /🤔inv [ID] -> [C][B][FFFFFF] Any Brother Random Player Invite Send
		-------------------------------
...
[00FF00] /🤔3 -> /🤔4 -> /🤔5 -> /🤔6""", uid
	                ...
	                clients.send(
		                    self.GenResponsMsg(
		                        f"""		-------------------------------
[00FF00َ] /🤔lag (team code)-> [C][B][FFFFFF] Any Player Groupt To laggin The Group 

[00FF00] /🤔attack (team code)-> [C][B][FFFFFF] Any Player Team To Send Join Attack Send

[40E0D0] /🤔start (team code)-> [C][B][FFFFFF] Any Brother Team Game Start Foorce TO sTART
		                       """, uid
	                    )
	                )
            ...
            if '1200' in data.hex()[0:4] and b'/lag' in data:
                try:
                    ...
                    clients.send(
                        self.GenResponsMsg(f"[C][B][00FF00]Your Order Confirm Done For Lag Team", uid)
                    )
            ...
            if "1200" in data.hex()[0:4] and b"/solo" in data:
                ...
                clients.send(
                    self.GenResponsMsg(
                        f"[C][B][00FF00] Group Bot Is Exit Done.  ", uid
                    )
                )
            if '1200' in data.hex()[0:4] and b'/attack' in data:
                try:
                    ...
                    if not command_parts:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]Using This To Attack Group Team  Example \n/attack [TeamCode]", uid))
                        continue
                    ...
                    clients.send(
                        self.GenResponsMsg(f"[C][B][FFA500]Is Team Code {team_code} Group Join Attack Join Stared", uid)
                    )
                    ...
                    clients.send(
                        self.GenResponsMsg(f"[C][B][00FF00]Dubble Attack Team Done! ✅   {team_code}!", uid)
                    )

                except Exception as e:
                    print(f"An error occurred in /attack command: {e}")
                    try:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]Attack Execute Error! ❌  .", uid))
                    except:
                        pass     
            ...
            if "1200" in data.hex()[0:4] and b"/start" in data:
                try:
                    ...
                    if not command_parts:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]Please provide Your Group Team Code.", uid))
                        continue
    ...
    def GET_LOGIN_DATA(self, JWT_TOKEN, PAYLOAD):
        ...
        headers = {
            ...
            'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 9; G011A Build/G011A)',
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="4207697700", password="E71FE984531619DC92F7539E95337690A3A8FF8D5055820C01F9A363F22B0489")
        client_thread.start()
    except Exception as e:
